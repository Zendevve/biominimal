      ...
      <div className={`w-full h-full overflow-y-auto no-scrollbar ${theme.bgClass} ${theme.textClass} ${fontClass}`}>
        <div className="px-6 py-12 flex flex-col items-center min-h-full">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm opacity-80 leading-relaxed max-w-[250px] mx-auto">
              {profile.bio}
            </p>
          </div>
          ...
          <div className="w-full space-y-3 flex-1">
            {profile.links.filter(l => l.isActive).map((link) => (
              <a
                ...
                rel="noreferrer"
                className={`
                  block w-full px-4 py-3.5 rounded-xl transition-all duration-200 flex items-center justify-between group
                  ${theme.cardBgClass} 
